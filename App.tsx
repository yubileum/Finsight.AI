  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fcfdff] font-['Inter']">
      ...
      <nav className="bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 flex justify-between h-20 items-center">
          <div className="flex items-center space-x-3 group cursor-pointer" onClick={() => window.location.reload()}>
            ...
            <span className="text-2xl font-black text-slate-900 tracking-tight">FinSight<span className="text-indigo-600">.AI</span></span>
          </div>
          <div className="flex items-center space-x-4">
            {transactions.length > 0 && (
              <button onClick={addStatement} className="hidden md:flex px-6 py-3 border border-indigo-100 bg-indigo-50 text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl text-indigo-700 hover:bg-indigo-100 transition-all shadow-sm items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
                Add Statement
              </button>
            )}
            <button onClick={startNewAudit} className="px-6 py-3 border border-slate-200 text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl text-slate-700 hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2">
               <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
               {transactions.length > 0 ? 'New Audit' : 'Upload Statement'}
            </button>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 lg:px-10 py-16">
        ...
        {isAnalyzing && (
          <div className="flex flex-col items-center justify-center py-40 bg-white rounded-[4rem] border border-slate-100 shadow-sm relative overflow-hidden">
            ...
            <div className="relative mb-12">
               ...
               <div className="absolute inset-0 flex items-center justify-center font-bold text-indigo-600 text-sm">AI</div>
            </div>
            <h3 className="text-4xl font-black text-slate-900 tracking-tight mb-4">Processing Financials</h3>
            <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-xs bg-slate-100 px-4 py-2 rounded-full animate-pulse">{loadingProgress}</p>
          </div>
        )}

        {!isAnalyzing && transactions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-48 bg-white rounded-[4rem] border border-slate-100 shadow-sm text-center px-10 relative overflow-hidden group">
             <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-50/40 via-white to-white pointer-events-none"></div>
            <div className="w-32 h-32 bg-white rounded-[3rem] flex items-center justify-center mb-12 rotate-3 group-hover:rotate-0 transition-transform shadow-xl shadow-indigo-100/50 border border-slate-50 relative z-10">
              <svg className="w-16 h-16 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
            </div>
            <h2 className="text-6xl font-black text-slate-900 mb-8 tracking-tighter relative z-10">Financial Analyzer</h2>
            <p className="text-slate-400 max-w-xl mb-16 text-2xl font-medium leading-relaxed relative z-10">Upload your PDF statement. We verify every decimal and reconcile totals with 100% precision.</p>
            <button onClick={startNewAudit} className="relative z-10 px-16 py-8 bg-indigo-600 text-white rounded-[3rem] font-black text-[12px] uppercase tracking-[0.4em] hover:bg-indigo-700 shadow-2xl shadow-indigo-200 transition-all hover:-translate-y-2 active:scale-95">Analyze Statement</button>
          </div>
        )}
        ...
        {transactions.length > 0 && !isAnalyzing && (
          <div className="space-y-16 animate-in fade-in duration-1000">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
              <div>
                <h2 className="text-7xl font-black text-slate-900 tracking-tighter">Report</h2>
                <div className="flex items-center gap-4 mt-4">
                  <p className="text-slate-400 text-2xl font-medium">Verified extraction & analysis</p>
                  <button onClick={addStatement} className="flex md:hidden px-4 py-2 border border-indigo-100 bg-indigo-50 text-[10px] font-black uppercase tracking-[0.2em] rounded-xl text-indigo-700 hover:bg-indigo-100 transition-all items-center gap-2">
              ...
              {!deepAnalysis && (
                <button onClick={handleDeepAnalysis} disabled={isThinking} className={`inline-flex items-center px-12 py-6 rounded-[3rem] font-black text-[10px] uppercase tracking-[0.3em] shadow-2xl transition-all hover:-translate-y-2 ${isThinking ? 'bg-slate-100 text-slate-400 cursor-wait' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
                  {isThinking ? (
                    <span className="flex items-center"><span className="w-2 h-2 bg-slate-400 rounded-full animate-bounce mr-2"></span>Processing...</span>
            ...
            <div className="pt-16"><TransactionTable transactions={transactions} /></div>
          </div>
        )}
      </main>

      <footer className="bg-white border-t border-slate-100 py-20 text-center">
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.6em]">FinSight Financial Intelligence 4.7</p>
      </footer>
    </div>
  );
};
