...
export const FinancialInsights: React.FC<FinancialInsightsProps> = ({ insights }) => {
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="bg-[#1e1b4b] text-white rounded-[3rem] p-10 lg:p-14 shadow-2xl relative overflow-hidden border border-indigo-900/50">
        ...
        <div className="relative z-10">
          <div className="flex items-center space-x-5 mb-8">
            <div className="bg-indigo-500/20 p-5 rounded-2xl border border-indigo-500/30 backdrop-blur-md">
              <svg className="w-10 h-10 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
            </div>
            <div>
              <h3 className="text-3xl font-black tracking-tight leading-none">Smart Audit</h3>
              <p className="text-indigo-200 text-sm font-bold uppercase tracking-widest mt-1">Executive Summary</p>
            </div>
          </div>
          <p className="text-2xl text-indigo-50 leading-relaxed font-medium opacity-90 max-w-5xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {insights.metrics.map((m, i) => (
          <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-lg transition-all group hover:-translate-y-1">
            <div className="bg-slate-50 w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-slate-400 mb-8 group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-colors shadow-sm">
              {IconMap[m.icon] || IconMap['chart']}
            </div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3">{m.label}</p>
            <p className="text-3xl font-black text-slate-900 tabular-nums tracking-tight">{m.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Actionable Tips */}
        <div className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
          <h4 className="text-2xl font-black text-slate-900 mb-10 flex items-center">
            <span className="bg-emerald-100 text-emerald-600 p-3 rounded-2xl mr-4">
          ...
          <div className="space-y-8">
            {insights.tips.map((tip, i) => (
              <div key={i} className="group relative pl-8 border-l-[3px] border-slate-100 hover:border-emerald-400 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <h5 className="font-black text-lg text-slate-800 tracking-tight">{tip.title}</h5>
                  <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-lg ${
                    tip.priority === 'High' ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-500'
                  }`}>
                    {tip.priority}
                  </span>
                </div>
                <p className="text-base text-slate-500 leading-relaxed font-medium">{tip.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
          <h4 className="text-2xl font-black text-slate-900 mb-10 flex items-center">
            <span className="bg-rose-100 text-rose-600 p-3 rounded-2xl mr-4">
          ...
          <ul className="space-y-6">
            {insights.redFlags.map((flag, i) => (
              <li key={i} className="flex items-start bg-rose-50/50 p-6 rounded-[2rem] border border-rose-100/50">
                <div className="w-2 h-2 rounded-full bg-rose-500 mt-2 mr-5 flex-shrink-0 shadow-sm shadow-rose-200"></div>
                <p className="text-base text-slate-800 font-bold leading-relaxed">{flag}</p>
              </li>
            ))}
            {insights.redFlags.length === 0 && (
              <div className="flex flex-col items-center justify-center h-48 bg-slate-50/50 rounded-[2rem] border border-dashed border-slate-200">
                ...
                <p className="text-slate-300 text-xs mt-1 uppercase tracking-widest">Clean Audit</p>
              </div>
            )}
          </ul>
        </div>
